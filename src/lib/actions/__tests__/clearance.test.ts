...
describe("Actions - Clearance", () => {
  ...
  describe("upgradeClearance", () => {
    ...
    it("successfully upgrades clearance", async () => {
      ...
      vi.mocked(prisma.user.findUnique).mockResolvedValueOnce({ clearanceLevel: 2, voltPoints: 500 } as any);
    ...
    it("throws specific error if updateMany fails (insufficient points)", async () => {
      vi.mocked(getServerSession).mockResolvedValueOnce({ user: { id: "user1" } } as any);
      vi.mocked(prisma.user.updateMany).mockResolvedValueOnce({ count: 0 } as any);
      
      // simulate user having not enough points
      vi.mocked(prisma.user.findUnique).mockResolvedValueOnce({ clearanceLevel: 1, voltPoints: 100 } as any);
    ...
    it("throws specific error if updateMany fails (already have clearance)", async () => {
      ...
      vi.mocked(prisma.user.findUnique).mockResolvedValueOnce({ clearanceLevel: 2, voltPoints: 2000 } as any);
    ...
    it("throws specific error if updateMany fails (needs previous tier)", async () => {
      ...
      vi.mocked(prisma.user.findUnique).mockResolvedValueOnce({ clearanceLevel: 0, voltPoints: 2000 } as any);
